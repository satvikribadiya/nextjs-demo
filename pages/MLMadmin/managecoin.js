...
export default function managecoin() {
    ...
    const onchangedata = (e) => {
        if (e.target.type === "file") {
            setonchange({ ...onchange, logo: e.target.files[0]?.name, theFiles: e.target.files[0] })
        } else {
    ...
    const submitdata = async () => {
        if (onchange.name === "") {
            toast.error("Enter coin name", { autoClose: 1500 })
            return false
        }
        if (onchange.name.length < 3) {
            toast.error("Coin name must be > 2 latters", { autoClose: 1500 })
            return false
        }
        if (!alphabetwithspace(onchange.name)) {
            toast.error("Coin name must be digits", { autoClose: 1500 })
            return false
        }
        if (onchange.ticker === "") {
            toast.error("Enter coin ticker", { autoClose: 1500 })
            return false
        }
        if (onchange.ticker.length < 3) {
            toast.error("Ticker must be > 2 latters", { autoClose: 1500 })
            return false
        }
        if (!alphabet(onchange.ticker)) {
            toast.error("Ticker must be without space letters", { autoClose: 1500 })
            return false
        }
        if (onchange.usdPrice == 0) {
            toast.error("Enter USD price", { autoClose: 1500 })
            return false
        }
        if (!number(onchange.usdPrice)) {
            toast.error("usdPrice must be number", { autoClose: 1500 })
            return false
        }
        if (onchange.logo === "") {
            toast.error("Add coin logo", { autoClose: 1500 })
            return false
        }
        if (onchange.status === "") {
            toast.error("Select coin status", { autoClose: 1500 })
            return false
        } 
        else {
            const formData = new FormData();
            formData.append('name', onchange.name);
            formData.append('ticker', onchange.ticker);
            formData.append('usdPrice', onchange.usdPrice);
            formData.append('logo', onchange.logo);
            formData.append('theFiles', onchange.theFiles);
            formData.append('status', onchange.status);
            formData.append('id', userid);

            const response = await fetch(process.env.MLM_API_BASEURL + 'coin/addcoin', {
                method: "POST",
                headers: {
    ...
    const getdata = async () => {
        const response = await fetch(process.env.MLM_API_BASEURL + 'coin/getcoindata', {
            method: "POST",
            body: JSON.stringify({ 
                startdate: st,
                enddate: ed,
    ...
    const editdata = async (id) => {
        handleShow()
        const response = await fetch(process.env.MLM_API_BASEURL + 'coin/geteditdata', {
            method: "POST",
    ...
    const switchstatuschange = async (id, status) => {
        ...
        const response = await fetch(process.env.MLM_API_BASEURL + 'coin/statuschange', {
            method: "POST",
                        ...
                        <div className="card">
                                    ...
                                    <div className='col-xl-12 col-lg-12 col-md-12 col-sm-12'>
                                        <div className='row justify-content-between'>
                                            ...
                                            <div className='col-xl-2 col-lg-2 col-md-2 col-sm-4 margintop mt-sm-3 mt-md-0 ft-right'>
                                                <button className="btn waves-effect waves-light btn-grd-primary float-right" onClick={handleShow}>Add Coin</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                            ...
                                            {coindata.length > 0 && coindata.map((post, da) => {
                                                return (
                                                    <tr key={da}>
                                                        ...
                                                        <td>{post.name}</td>
                                                        <td>{post.ticker}</td>
                                                        <td><Image src={`/assets/MLMadmin/allimage/${post.logo}`} height={50} width={50} alt="logo coin"></Image></td>
                                                        <td>{post.usdPrice}</td>
                                                        <td>
                            ...
                            <Modal show={show} onHide={handleClose}
                                ...
                                <Modal.Header closeButton>
                                    <Modal.Title>{userid === "" ? "Add" : "Edit"} Coin</Modal.Title>
                                </Modal.Header>
                                <Modal.Body>
                                    <div className="form-group row">
                                        <label className="col-sm-12 col-form-label">Coin Name</label>
                                        <div className="col-sm-12">
                                            <input type="text" className="form-control" placeholder="Enter coin name" name='name' value={onchange.name} onInput={(e) => e.target.value = e.target.value.replace(/[^A-Za-z ]/ig, '').replace(/(\..*)\./g, "$1")} onChange={(e) => onchangedata(e)} />
                                        </div>
                                    </div>
                                    <div className="form-group row">
                                        <label className="col-sm-12 col-form-label">Ticker Name</label>
                                        <div className="col-sm-12">
                                            <input type="text" className="form-control" placeholder="Enter coin name" name='ticker' value={onchange.ticker} onInput={(e) => e.target.value = e.target.value.replace(/[^A-Za-z]/ig, '').replace(/(\..*)\./g, "$1")} onChange={(e) => onchangedata(e)} />
                                        </div>
                                    </div>
                                    <div className="form-group row">
                                        <label className="col-sm-12 col-form-label">Logo</label>
                                        <div className="col-sm-12">
                                            <input type="file" className="form-control" placeholder="Logo" onChange={(e) => onchangedata(e)} />
                                        </div>
                                    </div>
                                    <div className="form-group row">
                                        <label className="col-sm-12 col-form-label">USD Price&nbsp;($)</label>
                                        <div className="col-sm-12">
                                            <input type="text" className="form-control" placeholder="USD Price" name='usdPrice' value={onchange.usdPrice} onInput={(e) => e.target.value = e.target.value.replace(/[^0-9 ]/ig, '').replace(/(\..*)\./g, "$1")} onChange={(e) => onchangedata(e)} />
                                        </div>
                                    </div>
                                    <fieldset>
